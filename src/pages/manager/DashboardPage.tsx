...
import { ArrowRight, Bot, ChevronLeft, ChevronRight, RotateCcw, Send, Sparkles } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [aiOpen, setAiOpen] = useState(true); // true = 45% chat, false = 25% chat
  ...
  return (
    <div className="flex rounded-xl overflow-hidden border border-slate-200 shadow-sm bg-white relative" style={{ height: h }}>

      {/* ───────────────── LEFT · Dashboard ───────────────── */}
      <div className="flex-1 min-w-0 overflow-y-auto border-r border-slate-200 transition-all duration-300">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
            {[
              ...
              { label: 'Total Revenue',  value: totalRevenue > 0 ? `RM ${totalRevenue.toLocaleString('en-MY', { minimumFractionDigits: 0 })}` : '—', sub: 'from completions', color: 'text-violet-600', dot: 'bg-violet-500' },
            ].map(card => (
              <div key={card.label} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-slate-300 transition-colors">
                <div className="flex items-center gap-2 mb-3">
                  <span className={`h-2 w-2 rounded-full ${card.dot}`} />
                  <span className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">{card.label}</span>
                </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              {techStats[0] && (
                <span className="text-xs text-slate-500">Top: <span className="font-semibold text-slate-700">{techStats[0].name}</span></span>
              )}
            </div>
            {techStats.length === 0 ? (
              ...
              <div className="space-y-2">
                {techStats.map((t, i) => {
                  ...
                  return (
                    <div key={t.name} className="flex items-center gap-3 py-2.5 px-4 rounded-xl border border-slate-100 bg-slate-50 hover:border-slate-200 transition-colors">
      ...
      {/* Toggle tab on the divider */}
      <button
        onClick={() => setAiOpen(o => !o)}
        title={aiOpen ? 'Slim chat (25%)' : 'Expand chat (45%)'}
        className="absolute top-1/2 -translate-y-1/2 z-20 flex items-center justify-center w-5 h-12 rounded-l-lg bg-blue-600 text-white shadow-md hover:bg-blue-700 transition-all duration-300"
